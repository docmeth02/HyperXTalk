...
/************************************************************************
 * HyperXTalk / LiveCode custom extensions                              *
 * These functions are not part of upstream libxml2. They add an        *
 * xmlParserOption bitmask parameter to the legacy SAX parse helpers    *
 * so that callers can pass XML_PARSE_HUGE, XML_PARSE_RECOVER, etc.     *
 ************************************************************************/

/**
 * xmlSAXParseFileWithDataAndOptions:
 * @sax:     SAX handler block (or NULL for default DOM building)
 * @filename: path to the XML file (or "-" for stdin)
 * @options:  bitmask of xmlParserOption values
 * @data:    user data stored in ctxt->_private (may be NULL)
 *
 * Parse an XML file using the given SAX handler and parser options.
 *
 * Returns the resulting document tree, or NULL on failure.
 */
xmlDoc *
xmlSAXParseFileWithDataAndOptions(xmlSAXHandler *sax, const char *filename,
...
/**
 * xmlSAXParseMemoryWithDataAndOptions:
 * @sax:     SAX handler block (or NULL for default DOM building)
 * @buffer:  pointer to the XML document in memory
 * @size:    size of the buffer in bytes (must not include a trailing NUL)
 * @options:  bitmask of xmlParserOption values
 * @data:    user data stored in ctxt->_private (may be NULL)
 *
 * Parse an in-memory XML document using the given SAX handler and
 * parser options.
 *
 * Returns the resulting document tree, or NULL on failure.
 */
xmlDoc *
xmlSAXParseMemoryWithDataAndOptions(xmlSAXHandler *sax, const char *buffer,
