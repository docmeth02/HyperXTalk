...
XMLPUBFUN xmlDoc *
		xmlSAXParseFileWithData	(xmlSAXHandler *sax,
					 ...
					 int recovery,
					 void *data);
/* HyperXTalk/LiveCode extension: SAX parse with explicit options bitmask */
XMLPUBFUN xmlDoc *
		xmlSAXParseFileWithDataAndOptions(xmlSAXHandler *sax,
					 const char *filename,
					 int options,
					 void *data);
XMLPUBFUN xmlDoc *
		xmlSAXParseMemoryWithDataAndOptions(xmlSAXHandler *sax,
					 const char *buffer,
					 int size,
					 int options,
					 void *data);
